...
--
-- Host: 127.0.0.1
-- Tempo de geração: 19/03/2024 às 05:08
...
--
-- Banco de dados: `radioscape`
--
CREATE DATABASE IF NOT EXISTS `radioscape` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
USE `radioscape`;

-- --------------------------------------------------------

--
-- Estrutura para tabela `cenario1`
--
...
--
-- Despejando dados para a tabela `cenario1`
--

INSERT INTO `cenario1` (`idPuzzle`, `pagina`, `nome`, `idade`, `sintomas`, `tomografia`, `doenca`) VALUES
...
--
-- Estrutura para tabela `cenario2`
--
...
--
-- Despejando dados para a tabela `cenario2`
--
...
--
-- Estrutura para tabela `cenario2-pacientes`
--

CREATE TABLE `cenario2-pacientes` (
  `exame` varchar(50) NOT NULL,
  `idade` int(11) NOT NULL,
...
--
-- Despejando dados para a tabela `cenario2-pacientes`
--

INSERT INTO `cenario2-pacientes` (`exame`, `idade`, `genero`, `caso`) VALUES
...
--
-- Estrutura para tabela `initialroom`
--
...
--
-- Despejando dados para a tabela `initialroom`
--

INSERT INTO `initialroom` (`idPuzzle`, `pagina`, `pergunta`, `alternativa1`, `alternativa2`, `alternativa3`, `alternativa4`, `cid1`, `cid2`, `cid3`, `cid4`) VALUES
...
--
-- Despejando dados para a tabela `puzzle`
--

INSERT INTO `puzzle` (`idPuzzle`, `resposta`, `caderno`, `resposta2`, `postit`) VALUES
(1, 'C07C34N23', '', '0', ''),
(2, '5182', 'crescente', '0', ''),
...
(2, 'teste', 'teste@gmail.com', '$2y$10$oDGVs9IkarqTXjamsrTe2eTTSt0lMRzKZu0dTz2xZOZmz22BHh9xa'),
(8, 'hehe', 'teste2@gmail.com', '$2y$10$KRvEA/cy1kNf5C1VjIl4.uSBI1A2KBkfhUFZ08WhjUCiWAXDgXiqm'),
(10, 'hehe', 'teste234@gmail.com', '$2y$10$SZ4Dvj6BKpO5LU4frTNFk.iImn0.IWp5Z7kLBG.rlBS6039OUjIgC'),
(11, 'dede', 'l@gmail.com', '$2y$10$gbgI6nzzZ5.85TQZ40scsez5c7WhBlxWPMJXzdTqd5fAqqUWX6332');
...
--
-- Índices de tabela `cenario1`
--
ALTER TABLE `cenario1`
  ADD KEY `fk` (`idPuzzle`);

--
-- Índices de tabela `cenario2`
--
ALTER TABLE `cenario2`
  ADD UNIQUE KEY `pacUni` (`pacienteCodigo`),
  ADD KEY `fk_cenario2` (`idPuzzle`),
  ADD KEY `fk_exame` (`exame`);

--
-- Índices de tabela `cenario2-pacientes`
--
ALTER TABLE `cenario2-pacientes`
  ADD PRIMARY KEY (`exame`);

--
-- Índices de tabela `initialroom`
--
...
--
ALTER TABLE `puzzle`
  MODIFY `idPuzzle` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=26;
...
--
-- Restrições para tabelas `cenario1`
--
ALTER TABLE `cenario1`
  ADD CONSTRAINT `fk` FOREIGN KEY (`idPuzzle`) REFERENCES `puzzle` (`idPuzzle`);

--
-- Restrições para tabelas `cenario2`
--
ALTER TABLE `cenario2`
  ADD CONSTRAINT `fk_cenario2` FOREIGN KEY (`idPuzzle`) REFERENCES `puzzle` (`idPuzzle`),
  ADD CONSTRAINT `fk_exame` FOREIGN KEY (`exame`) REFERENCES `cenario2-pacientes` (`exame`);

--
-- Restrições para tabelas `initialroom`
--
ALTER TABLE `initialroom`
